 ...
 * Optimized for mobile/desktop hybrid environments.
 * Implements exponential backoff, Firestore batching, and model health tracking.
 */

const CORE_CONFIG = {
  BATCH_SIZE: 5,
  BUFFER_MAX: 50,
  CYCLE_INTERVAL: 45000,
  MODELS: [
...
export default function App() {
  ...
  // Keep metrics ref in sync for closure safety in the loop
  useEffect(() => { metricsRef.current = state.metrics; }, [state.metrics]);

  // Auth Implementation - Mandatory Rule 3
  useEffect(() => {
  ...
  // Data Sync Implementation - Mandatory Rule 1 & 2
  useEffect(() => {
    if (!user) return;
  ...
  const runCycle = useCallback(async () => {
    ...
    // Safety check for model blocking
    if (state.modelHealth[state.selectedModel]?.isBlocked) {
      ...
      if (nextAvailable) {
        ...
        pushLog("Global cooldown active. Stopping.", "error");
    ...
    try {
      ...
      if (!currentPath) { 
        pushLog("Queue Exhausted", "success");
        dispatch({ type: 'TOGGLE' }); 
        return; 
      }
      ...
      if (!optimized || optimized === raw) {
        outcome = 'SKIP';
        pushLog(`No change for ${currentPath.split('/').pop()}`);
      } else {
